...
export function MyBtn() {
  ...
  return (
    <div className="w-full h-56 gap-5 bg-overlay flex-col flex justify-center items-center">
      <div className="space-x-5">
        <span onClick={()=>setTheme('theme-first')} className={classnames("cursor-pointer transform",theme=='theme-first'&&'text-2xl')}>theme-first</span>
        <span onClick={()=>setTheme('theme-second')} className={classnames("cursor-pointer transform",theme=='theme-second'&&'text-2xl')}>theme-second</span>
      </div>
      <button
