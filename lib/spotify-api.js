...
//https://accounts.spotify.com/en/authorize?response_type=code&client_id=651388d70a8149658dc770cf641208e7&scope=user-read-playback-state
const my_refresh_token = process.env.SPOTIFY_REFRESH_TOKEN;
const Authorization = process.env.SPOTIFY_AUTHORIZATION;
let access_token = "";
    ...
    instance
      ...
      .then((data) => {
        let items = data.items
          ...
          .filter((e) => new Date() - new Date(e.played_at) < 60 * 30 * 1000);
        getAudioFeatures(items.map((e) => e.id)).then((data) => {
          resolve({ feeling: data.filter((e) => e != null).map((e) => e.valence) });
        });
      })
      .catch((e) => reject(e));
  });
...
export function getAudioFeatures(ids) {
  return new Promise((resolve, reject) => {
    instance
      .get("/audio-features?ids=" + encodeURI(ids))
      .then((data) => resolve(data.data.audio_features))
      .catch((e) => reject(e));
  });
}
